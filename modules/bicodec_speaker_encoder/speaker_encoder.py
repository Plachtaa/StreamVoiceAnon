...
class SpeakerEncoder(nn.Module):
    ...
    def tokenize_wav(self, wav: torch.Tensor, wav_lens: torch.Tensor) -> torch.Tensor:
        """tokenize the input wav"""
        mel = self.mel_transformer(wav).squeeze(1).mT
        _, features = self.speaker_encoder(mel, True)
        mel_lens = wav_lens // self.mel_transformer.hop_length
        mask = sequence_mask(mel_lens + 32, features.shape[2] + 32)
        x = self.perceiver_sampler(features.transpose(1, 2), mask=mask).transpose(1, 2)
        zq, indices = self.quantizer(x)
        return zq, indices
...
if __name__ == "__main__":
    ...
    waves = torch.randn(8, 1, 16000)
    wave_lens = torch.tensor([16000] * 8)
    zq, indices = model.tokenize_wav(waves, wave_lens)
