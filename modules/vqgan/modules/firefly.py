...
class FishConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.functional.pad(
            x, (self.kernel_size - self.stride, 0), mode="constant", value=0
        )
        return self.conv(x)
...
class FishTransConvNet(nn.Module):
    ...
    def forward(self, x):
        if self.stride == self.kernel_size // 2:
            x = nn.functional.pad(x, (1, 0), mode="constant", value=0)
        elif self.stride == self.kernel_size:
            x = nn.functional.pad(x, (1, 1), mode="constant", value=0)
        else:
            x = x
        return self.conv(x)
...
class ConvNeXtEncoder(nn.Module):
    def __init__(
        ...
        self.g_in = nn.Conv1d(gin_channels, input_channels, kernel_size=1) if gin_channels > 0 else None
